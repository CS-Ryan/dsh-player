...
示例:
  ...
  dsh-player session.jsonl -p 8080
  dsh-player session.jsonl.zstd --export player.html --mask
`)
}
...
async function main() {
  ...
  if (args.includes('-v') || args.includes('--version')) {
    ...
    console.log(`dsh-player v${pkg.version}`)
    process.exit(0)
  }
  ...
  try {
    ...
    console.error(`❌ 解析或启动播放失败: ${err.message}`)
    process.exit(1)
  }
}
